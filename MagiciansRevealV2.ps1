...
function Find-SuppliedCheatStrings {
    param([string]$Path)
    $hits=[System.Collections.Generic.List[object]]::new(); $zip=$null
    try {
        $zip=[IO.Compression.ZipFile]::OpenRead($Path)
        foreach($e in $zip.Entries){
            $chunks=[System.Collections.Generic.List[string]]::new(); $chunks.Add($e.FullName)
            if($e.Length -le 8MB -and $e.Length -gt 0){$ms=[IO.MemoryStream]::new();$e.Open().CopyTo($ms);$b=$ms.ToArray();$ms.Dispose();$chunks.Add([Text.Encoding]::UTF8.GetString($b));$chunks.Add([Text.Encoding]::ASCII.GetString($b))}
            foreach($chunk in $chunks){ foreach($form in Get-NormalizedForms $chunk){ foreach($sig in ($moduleNames+$clientSignatures+$literalCheatStrings)){ if($form.text.IndexOf($sig,[StringComparison]::OrdinalIgnoreCase) -ge 0){$hits.Add([pscustomobject]@{match=$sig;entry=$e.FullName;normalization=$form.method})} } foreach($obf in $knownCheatObfuscators.Keys){foreach($pat in $knownCheatObfuscators[$obf]){if($form.text.IndexOf($pat,[StringComparison]::OrdinalIgnoreCase) -ge 0){$hits.Add([pscustomobject]@{match=$pat;entry=$e.FullName;normalization=$form.method;obfuscator=$obf})}}} } }
        }
    } catch {} finally {if($zip){$zip.Dispose()}}
    @($hits | Sort-Object match,entry,normalization -Unique)
}
Invoke-Module 'minecraft_mods' {
    ...
    foreach($jar in $jars){ $hash=(Get-FileHash -LiteralPath $jar.FullName -Algorithm SHA256).Hash.ToLowerInvariant(); if($KnownHashes.ContainsKey($hash)){ $r=$KnownHashes[$hash]; Add-Flag ([string]$r.tier) 'minecraft.known_mod_hash' 'minecraft' 'Known mod hash matched' "The JAR matches the signature catalog entry '$($r.cheat_name)'." @{path=$jar.FullName;sha256=$hash} }; $hits=@(Find-SuppliedCheatStrings $jar.FullName); if($hits.Count -gt 0){$names=@($hits|Select-Object -ExpandProperty match -Unique);$obfs=@($hits|Where-Object obfuscator|Select-Object -ExpandProperty obfuscator -Unique); Add-Flag Detection 'minecraft.jar_signature' 'minecraft' 'Supplied cheat string detected in Minecraft JAR' "Detected $($names.Count) supplied cheat/client indicator(s) in $($jar.Name): $([string]::Join(', ',($names|Select-Object -First 25)))." @{path=$jar.FullName;sha256=$hash;matches=$hits;obfuscators=$obfs;all_supplied_strings_tested=$true} 'minecraft_mods'} }
}
